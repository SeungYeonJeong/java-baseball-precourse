...
public class Application {
    ...
    // 스트라이크, 볼 개수 반환
    public int[] compare(String[] inputValue, String[] randomValue) {
        int[] counts = new int[2];

        for(int i = 0; i < 3; i++){
            for(int j = 0; j < 3; j++){
                counts = getCounts(inputValue, randomValue, i, j, counts);
            }
        }
    ...
    // 스트라이크, 볼 개수 계산
    public int[] getCounts(String[] inputValue, String[] randomValue, int i, int j, int[] counts){
        if (i == j && inputValue[i].equals(randomValue[j])){
            counts[0]++;
        }
        else if (i != j && inputValue[i].equals(randomValue[j])){
            counts[1]++;
        }
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            int[] counts = application.compare(inputNumberArr, randomNumberArr);
